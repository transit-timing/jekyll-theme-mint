...
#import corner
# This script contains code to process 2-minute and 30-minute TESS light curves
...
def analyze_system(params_system, flux, time):
    '''
    Process TESS light curve (2-minute, 10-minute, or 30-minute cadence) and estimate mid-transit times
    '''
...
'/'
    ...
    ##################################################################################
    # select transit data from orbits that have at least the minimum number of data points
    ##################################################################################
    ...
    ##################################################################################
    # Detrend each individual transit, fold the data and find a preliminary transit model (before we omit any orbits with high MAD)
    ##################################################################################
    ...
    for n in orbits:
        ...
        #print('initial_flux_uncertainties ', initial_flux_uncertainties)
    ...
    if no > 2: # check that there are at leasat 3 transits in the light curve
        print('Before applying MAD mask')
        i = 0
        for n in orbits:
            ...
            #plt.plot(tp-tmid, fp, '.')
            #plt.xlabel(f'Raw data at orbit {n}')
            #plt.show()
            ...
            #fp_calc_check = transit_lightcurve(popt, tp)

            #plt.figure()
            #plt.plot(tp-tmid, fp_calc_check, '.r')
            #plt.plot(tp-tmid, f_detrended, '.b')
            #plt.title('Initial de-trended event and best fit')
            #plt.show()
            ...
            #print('transit_residuals ', transit_residuals)
            #plt.figure()
            #plt.plot(tp-tmid, f_detrend, '.b')
            #plt.plot(tp-tmid, fp_calc, '.r')
            #plt.xlabel('WTF')
            #plt.show()
    ...
    oibeo_stage2_ = []
    flux_data_stage2_ = []
    # determine the shape of the time series array before high MAD events were removed
    print('initial_detrended_flux.shape: ', initial_detrended_flux.shape)
    print('t.shape: ', t.shape)
    print('initial_flux_uncertainties.shape: ', initial_flux_uncertainties.shape)
    ...
    # determine the shape of the time series array after high MAD events were removed
    print('stage 2 shapes: ')
    print('oibeo_stage2_.shape: ', oibeo_stage2.shape)
    print('flux_uncertainty_stage2.shape: ', flux_uncertainty_stage2.shape)
    print('time_stage2.shape: ', time_stage2.shape)
    print('detrended_flux_stage2.shape: ', detrended_flux_stage2.shape)
    print('flux_data_stage2_.shape: ', flux_data_stage2.shape)
    ...
    print('Making sure shapes at stage 2 match:')
    print(t_folded2.shape, detrended_flux_stage2.shape, flux_uncertainty_stage2.shape)
    ...
    for n in orbits_stage2:
        ...
        #corner.corner.savefig('/Users/kate/Desktop/test.png')

        #mcmc_corner = np.concatenate(coeff_concatenated_chain, t0_concatenated_chain, axis = 1)
        #figure = corner.corner(mcmc_corner)
        #corner.corner.savefig('/Users/kate/Desktop/test1.png')
        #print('mcmc_corner.shape ', mcmc_corner.shape)
        ...
        print('Coeffs estimated with MCMC: ', coeff_mcmc)
        #print('coeff_concatenated_chain.shape', coeff_concatenated_chain.shape)
        #print('t0_concatenated_chain.shape', t0_concatenated_chain.reshape(t0_concatenated_chain.shape[0],1).shape)
        #walkers = np.concatenate((coeff_concatenated_chain, t0_concatenated_chain.reshape(t0_concatenated_chain.shape[0],1)), axis = 1 )
        #print('walkers.shape', walkers.shape)

        #fig_corner = plt.figure()
        #figure_corner = corner.corner(walkers, quantiles=(0.16, 0.84))#, labels=[r"$a$", r"$b$", r"$T_0$"])
        
        #corner.overplot_lines(figure_corner, value1, color="C1")
        #corner.overplot_points(figure_corner, value1[None], marker="s", color="C1")
        #corner.overplot_lines(figure, value2, color="C2")
        #corner.overplot_points(figure, value2[None], marker="s", color="C2")
        #plt.savefig(dirname + name + f'_d_corner_orbit_{i}.png')
        ...
        #ax.set_xlim((tp-t0_mcmc).min()-0.1, (tp-t0_mcmc).max()+0.1)
        ax.set_ylim(min(f_detrend)-0.003*min(f_detrend), max(f_detrend)+0.003*max(f_detrend))
        #ax.set_ylim(min(f_detrend)-0.02, max(f_detrend)+0.02)
        fig.add_subplot(ax)
    ...
    # the filtering for which events to include consists of 1) min number of points requrement; 
    ...
    #plt.axis.xaxis.set_major_locator(MaxNLocator(integer=True))
