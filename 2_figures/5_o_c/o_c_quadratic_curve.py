...
for planet_name in os.listdir(path2database):
  ...
  if True:
    ...
    linear_model = period*orbit_number_arr + t0
    ...
    if np.sum(source == 'TESS') == 0:   
        ...
        fig = plt.figure()
        plt.plot(orbit_number_arr, quadratic_residuals*24*60, 'k')
        ...
        plt.title(f'{planet_name}:' + rf' $P = {per}({u_per}), T_0 = {t0}({u_t0})$',  fontsize=24)
        plt.xlabel('Orbit Number',  fontsize=24)
        plt.ylabel(r'$\Delta t$ [min]',  fontsize=24)
        ...
        axs[0].set_title(f'{planet_name}:' + rf' $P = {per}({u_per}), T_0 = {t0}({u_t0})$')
