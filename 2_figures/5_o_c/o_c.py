...
for planet_name in os.listdir(path2database):
  ...
  if np.sum(source == 'TESS') == 0:   
      y_min = (o_c[m] - t_err[m]).min() - 1/(60*24)
      y_max = (o_c[m]+t_err[m]).max() + 1/(60*24)
      ...
      if  n > 1:
        ...
        print(planet_name, ': ', o_c[(o_c  > y_max) | (o_c < y_min)])
      elif n==1:
      ...
      y_min = (o_c[m] - t_err[m]).min() - 3.5/(60*24)
      y_max = (o_c[m]+t_err[m]).max() + 3.5/(60*24)
