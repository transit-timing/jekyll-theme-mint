...
directories = ['planets_>1_no_tess', 'planets_tess/tt_output_nov15']
...
P_T0_linear_tess_targets = pd.DataFrame(columns = ['System', 'N', 'Chi-square', 'sigma_0', 'Period (days)', 'P error', 'T0 (BJD TDB)', 'T0 error'], data = ephemerides_linear_tess_targets)
P_T0_linear_tess_targets.sort_values(["System"], ascending=True).to_csv('/Users/kate/Documents/research/paper/3_tables/2_ephemerides/ephemerides_tess_only.csv', index = False)

P_T0_dPdE = pd.DataFrame(columns = ['System', 'P (quadraitc model)', 'P error (quadraitc model)', 'dP/dE (quadraitc model) msec/yr', 'dP/dE error (quadraitc model)  msec/yr', 'T0 (quadraitc model)', 'T0 error (quadraitc model)'], data = ephemerides_quadratic)
P_T0_dPdE = P_T0_dPdE.iloc[: , 1:]

P_T0_dPdE = pd.concat([P_T0_linear, P_T0_dPdE], axis=1)
sorted_P_T0_dPdE = P_T0_dPdE.sort_values(["System"], ascending=True)
sorted_P_T0_dPdE.to_csv('/Users/kate/Documents/research/paper/3_tables/2_ephemerides/ephemerides.csv', index = False)
 
quadratic_fit_preferred_df = pd.DataFrame(columns=['System', 'BIC quadraitc', 'BIC linear'], data = quadratic_fit_preferred)
...
significant_dpde_df = pd.DataFrame(columns=['System', 'dP/dE','dP/dE error', 'Status'], data = significant_dpde)
significant_dpde_df.to_csv('/Users/kate/Documents/research/paper/3_tables/2_ephemerides/significant_dpde.csv', index = False)

 
all_chi_square = pd.DataFrame(columns=['System', 'chi-square','N dof', 'Reduced chi-square'], data = chi2s)
