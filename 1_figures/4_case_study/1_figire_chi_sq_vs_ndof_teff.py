...
# Color points in chi square vs ndof by effective temperature. Can't extract some Teff because of different name convention (zeros (wasp-50 vs wasp-050), CoRoT vs COROT, etc)
...
teffs = []
df = pd.read_csv('/Users/kate/Documents/research/paper/3_tables/allplanets-csv.csv')
for folder in os.listdir('/Users/kate/Documents/research/paper/1_database/reviewed_planets_clean/'):
  try:
    ...
    if np.sum(m) > 0:
        ...
        try:
            teff = df.loc[df['System '] == planet_name.upper(), 'Teff '].iloc[0] 
            teffs.append(teff)
        except:
            teffs.append(0)
...
mm = chi_squares > 1000
big_chi = chi_squares[mm]
big_ndofs = ndofs[mm]
print('Targets with very large chi square: ', planet_names[mm])
...
print('shapes ', teffs.shape, chi_squares_tess.shape)
 

# fraction of targets with TESS data analyzed for which the chi square > ma_chi_square
max_chi_mask = chi_squares_tess > ndofs_tess + 3*np.sqrt(2*ndofs_tess)
print('fraction of targets for which chi square > max_chi_square: ', chi_squares_tess[max_chi_mask].shape[0]/chi_squares_tess.shape[0])
...
df_tess = pd.DataFrame({'ndofs_tess': ndofs_tess, 'chi_squares_tess': chi_squares_tess,  'teff': teffs})
#print(df) 
n_min = np.min(ndofs)
n_max = np.max(ndofs)
...
chi_max_tess = ndofs_tess + 3*np.sqrt(2*ndofs_tess)
chi_min_tess = ndofs_tess - 3*np.sqrt(2*ndofs_tess)
...
axes[0].plot(big_ndofs, big_chi, '.', color = 'red')
...
axes[0].fill_between(ndofs_arr, y1, y2,  color = 'gray', alpha = 0.2)
axes[0].set_xscale('log')
axes[0].set_yscale('log')

g2= sns.scatterplot(ax=axes[1],x='ndofs_tess', y='chi_squares_tess', data = df_tess, hue = 'teff', palette='bwr', alpha = 0.9, edgecolor="black", linewidth=0.9)
...
axes[1].fill_between(ndofs_tess_arr, y1_tess, y2_tess,  color = 'gray', alpha = 0.2)
