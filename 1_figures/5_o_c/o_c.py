...
for folder in os.listdir('/Users/kate/Documents/research/paper/1_database/reviewed_planets_clean/'):
  try:
    ...
    if np.sum(source == 'Our work') == 0:
        ...
        model = t0 + P * orbit_number_after_sigma_clip
        ...
        o_c_linear = t_after_sigma_clip - (t0 + P * orbit_number_after_sigma_clip)
        ...
        plt.savefig(f'/Users/kate/Documents/research/paper/2_figures/o_c_figures/o_c/{planet_name}_o_c.png')
        #plt.show()
        plt.close(fig)
        ...
        #factor = np.sqrt(chi_lit/(ndof - chi_tess))
     
        # calculate factor by which to enlarge literature uncertainties
        if ndof > 0:
          if t_tess.shape[0]>=1:
            if chi_lit/(ndof - chi_tess) > 1:
              factor = np.sqrt(chi_lit/(ndof - chi_tess))
            else: factor = 1
          else:
            if chi_lit/ndof > 1:
              factor = np.sqrt(chi_lit/ndof)
            else: factor = 1    
        else:
          factor = 1     
        ...
        #plt.plot(orbit_number, o_c*24*60, '.')  
